...
# Should be the same as CFK7 but attacking one of the bottlenecks locate using line_profiler:
#pt = yind.sum() seems to account for 9.5% of the total processing time



@cython.boundscheck(False)
@cython.wraparound(False)
cdef int getSum(int[:] arr, int arrLen) nogil:
...
#    cdef int[:] yind_mv
    ...
    for t in range(0, n):
        ...
        yind = ~np.isnan(y[t,:])
#        pt = yind.sum()
#        yind_mv = yind.astype(int)
        pt = getSum(yind.astype(int), p)
        yt[:pt] = y[t,yind]
        Zt[:pt,:] = Z[yind,:]
